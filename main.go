...
func main() {
	...
	mux.HandleFunc("/api/cib/create-primitive/", authMiddleware(api.CreatePrimitiveHandler))
	mux.HandleFunc("/api/cib/update-primitive/", authMiddleware(api.UpdatePrimitiveHandler)) // Can't find where it was used (let's not remove it though)
	mux.HandleFunc("/api/cib/rename-primitive/", authMiddleware(api.RenamePrimitiveHandler))
	mux.HandleFunc("/api/cib/delete-primitive/", authMiddleware(api.DeletePrimitiveHandler))
